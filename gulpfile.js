...
gulp.task('tsc-watcher-client', function () {
    gulp.watch(config.ts.client, ['tsc-client']);
});

gulp.task('tsc-watcher-server', function () {
    gulp.watch(config.ts.server, ['tsc-server']);
});
...
function runTSC(directory, done) {
    ...
    var childProcess = cp.spawn('node', [tscjs, '-p', directory],
        { cwd: process.cwd() });
